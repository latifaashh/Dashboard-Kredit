...
def find_col(df, candidates):
    ...
    Prioritas exact match, kemudian partial match.
    """

    lower_map = {}

    for c in df.columns:
        lower_map.setdefault(
            str(c).lower().strip(),
            c
        )
...
# ============================================================
# VALIDASI PINJAMAN ID
# ============================================================

if pid is None:

    st.error(
        "Kolom 'pinjaman_id' tidak ditemukan."
    )
...
st.caption(
    "Analisis kualitas portofolio kredit berdasarkan "
    "NPL, DPD, agunan, tunggakan, dan collection rate."
)
...
total_kewajiban = (
    df[kewajiban].sum()
    if (
        kewajiban is not None
        and kewajiban in df.columns
    )
    else 0
)
...
total_realisasi = (
    df[realisasi].sum()
    if (
        realisasi is not None
        and realisasi in df.columns
    )
    else 0
)
...
if (
    hari_keterlambatan is not None
    and hari_keterlambatan in df.columns
):

    avg_dpd = (
        df[hari_keterlambatan]
        .mean()
    )
...
st.header(
    "NPL Berdasarkan Cabang"
)

st.caption(
    "Cabang dengan tingkat kredit bermasalah tertinggi."
)
...
if nama_cabang is not None:
    ...
    st.info(
        "Kolom nama cabang tidak tersedia."
    )
...
st.caption(
    "Rata-rata hari keterlambatan pembayaran angsuran "
    "untuk setiap produk kredit."
)
...
if (
    ...
    if len(dpd_produk) > 0:
        ...
        with b:
            ...
            st.metric(
                ...
                f"{worst_product['rata_rata_DPD']:.2f} hari"
            )
    ...
    st.warning(
        "Kolom 'hari_keterlambatan' belum tersedia "
        "pada fact_pinjaman_final.csv. "
        "DPD 102,77 hari dari detail_angsuran "
        "belum dapat dihitung dari file ini."
    )
...
st.header(
    "NPL Berdasarkan Status Agunan"
)

st.caption(
    "Perbandingan tingkat NPL antara pinjaman "
    "dengan agunan dan tanpa agunan."
)
...
if agunan is not None:
    ...
    st.info(
        "Kolom nilai agunan tidak tersedia."
    )
...
with left:
    ...
    if (
        nama_petugas is not None
        and baki is not None
    ):
...
with right:
    ...
    if (
        ...
        st.info(
            "Kolom jenis usaha atau baki debet "
            "tidak tersedia."
        )
...
st.header(
    "Collection Rate Berdasarkan Cabang"
)

st.caption(
    "Cabang dengan tingkat collection rate terendah."
)
...
if (
    ...
    if len(branch_collection) > 0:
        ...
        st.info(
            "Tidak ada data collection rate."
        )
    ...
    st.info(
        "Kolom cabang, realisasi, atau kewajiban "
        "tidak tersedia."
    )
...
st.caption(
    f"Total portofolio: "
    f"{total_pinjaman_awal:,} pinjaman | "
    f"Sedang dianalisis: "
